...
CREATE TABLE IF NOT EXISTS "user" (
    ...
    level int NOT NULL DEFAULT 0 COMMENT 'Niveau de l''utilisateur (pour un jeu ou une compétence)',
    experience int NOT NULL DEFAULT 0 COMMENT 'Expérience de l''utilisateur (pour un jeu ou une compétence)',
    is_admin BOOLEAN DEFAULT FALSE COMMENT 'Indique si l''utilisateur a un rôle d''administrateur',
...
CREATE TABLE IF NOT EXISTS "event" (
    ...
    category_id int NOT NULL REFERENCES category(id) COMMENT 'Référence à la catégorie de l''événement',
    organizer_id int NOT NULL REFERENCES "user"(id) COMMENT 'Identifiant de l''organisateur de l''événement',
...
CREATE TABLE IF NOT EXISTS "user_has_friend" (
    user_id int NOT NULL REFERENCES "user"(id) COMMENT 'Identifiant de l''utilisateur',
    friend_id int NOT NULL REFERENCES "user"(id) COMMENT 'Identifiant de l''ami de l''utilisateur',
...
CREATE TABLE IF NOT EXISTS "friend_request" (
    sender_id int NOT NULL REFERENCES "user"(id) COMMENT 'Identifiant de l''utilisateur envoyant la demande',
    receiver_id int NOT NULL REFERENCES "user"(id) COMMENT 'Identifiant de l''utilisateur recevant la demande',
...
CREATE TABLE IF NOT EXISTS "event_has_participant" (
    event_id int NOT NULL REFERENCES event(id) COMMENT 'Identifiant de l''événement',
    participant_id int NOT NULL REFERENCES "user"(id) COMMENT 'Identifiant du participant',
...
CREATE TABLE IF NOT EXISTS "event_invitation" (
    event_id int NOT NULL REFERENCES event(id) COMMENT 'Identifiant de l''événement',
    organizer_id int NOT NULL REFERENCES "user"(id) COMMENT 'Identifiant de l''organisateur de l''événement',
    invited_friend_id int NOT NULL REFERENCES "user"(id) COMMENT 'Identifiant de l''ami invité',
